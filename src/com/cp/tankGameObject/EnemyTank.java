...
/**
 * 敌人坦克	继承自Tank类，并且实现Runnable接口
 * 敌人坦克的线程是每隔36毫秒行走的，各种判断（30毫秒）会在这36毫秒以内判断完毕
 * @author 1052067939
 * @version	1.0
 * @since	JavaSe-1.6
 *
		...
		while(true){
			...
			case EnemyTank.NORTH:
				for(;;){		
					...
					//如果现在坦克的方向不是北方，跳出
					if(this.getDirect()!=Tank.NORTH) break;
					//如果不重叠，前进
			...
			case EnemyTank.SOUTH:
				for(;;){
					...
					if(this.getDirect()!=Tank.SOUTH) break;
					if(this.isOverlapYes()==false)
					this.goSouth();
				}
				break;
			case EnemyTank.WEST:
				for(;;){
					...
					if(this.getDirect()!=Tank.WEST) break;
					if(this.isOverlapYes()==false)
					this.goWest();
				}
				break;
			case EnemyTank.EAST:
				for(;;){
					...
					if(this.getDirect()!=Tank.EAST) break;
					if(this.isOverlapYes()==false)
					this.goEast();
				}
				break;
