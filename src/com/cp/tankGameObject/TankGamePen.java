...
public class TankGamePen {
	...
	public void drawNorth(Graphics g,Stuff tank,JPanel panel){
	 /* int x=tank.getX();
		...
		g.setColor(Color.white);
		//7.画出中间2矩形
		g.draw3DRect(x-10, y-9, 20, 18,false);
		//8.画出中间3矩形
		...
		g.setColor(tank.getColor());
		//10.画矩形
		g.fill3DRect(x-3, y-12, 6, 3, false);
		g.fill3DRect(x-2, y-20, 4, 2, false);
		g.fill3DRect(x-1, y-20, 2, 11, false);
		*/
		Image image;
	...
	public void drawSouth(Graphics g,Stuff tank,JPanel panel){
		...
		g.fill3DRect(x-2, y+18, 4, 2, false);
		*/
		Image image;
		if(tank.getType2()==Tank.MY){
			g.setColor(Color.green);
	...
	public void drawWest(Graphics g,Stuff tank,JPanel panel){
		...
		g.fill3DRect(x-20, y+10, 40, 10,false);
		g.setColor(tank.getColor());
		for(int i=0;i<20-1;i++){
		...
		g.draw3DRect(x-5, y-3, 10, 6,false);
		g.drawLine(x-15, y-14, x-10, y-9);
		g.drawLine(x+15, y-14, x+10, y-9);
		g.drawLine(x-15, y+14, x-10, y+9);
		g.drawLine(x+15, y+14, x+10, y+9);
		g.setColor(tank.getColor());
		g.fill3DRect(x-12, y-3, 3, 6, false);
		g.fill3DRect(x-20, y-1, 11, 2, false);
		g.fill3DRect(x-20, y-2, 2, 4, false);
		*/
		Image image;
		if(tank.getType2()==Tank.MY){
	...
	public void drawEast(Graphics g,Stuff tank,JPanel panel){
		...
		g.drawLine(x+15, y+14, x+10, y+9);
		g.setColor(tank.getColor());
		g.fill3DRect(x+9, y-3, 3, 6, false);
		g.fill3DRect(x+9, y-1, 11, 2, false);
		g.fill3DRect(x+18, y-2, 2, 4, false);
		*/
		Image image;
		if(tank.getType2()==Tank.MY){
	 ...
	 * @param tgp	游戏主要面板对象
	 */
	public void drawRight(Graphics g,TankGamePanel tgp){
		for(int i=0;i<tgp.getTgc().getEnemyTankNum();i++){
			if(i>=4){
		...
		for(int j=0;j<tgp.getTgc().getMyTankNum();j++){
			g.drawImage(TankGameImages.myTankImg[Stuff.NORTH], 602+50*j, 400, 40, 40, tgp);
		}
		g.drawString("我的坦克子弹数目:"+tgp.getTgc().getMyBulletNum(), 620, 500);
	}
}
