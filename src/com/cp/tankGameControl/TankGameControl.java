...
public class TankGameControl {	
	 ...
	 * 我杀死的敌人坦克数量0辆
	 */
	public TankGameControl() {
		this.enemyTankNum=8;
		this.myTankNum=4;
		this.myBulletNum=TankGameControl.STARTBULLETNUM;
		this.beKilled=0;
	}
	...
	public void judge(Vector<MyTank> myTanks,Vector<EnemyTank> enemys,Map map,Vector<Bomb> bombs){
		...
		for(int j=0;j<myTanks.size();j++){									//从我的坦克容器中取出一辆我的坦克
			...
			for(int i=0;i<enemys.size();i++){								//从敌人的坦克容器中取出一辆敌人的坦克
				...
				if(enemyTank.getY()>=20) enemyTank.setInMap(true);
				...
				for(int k=0;k<enemyTank.getBullets().size();k++){
					...
					for(int l=0;l<map.getBricks().size();l++){				//取出每个砖块对象与子弹比较
						...
						if(Math.abs(eb.getX()-brick.getX())<=brick.getWidth()/2&&Math.abs(eb.getY()-brick.getY())<=brick.getWidth()/2){//子弹击中砖块
					...
					for(int l=0;l<map.getIrons().size();l++){				//取出没个铁块对象与子弹比较
						...
						if(Math.abs(eb.getX()-iron.getX())<=iron.getWidth()/2&&Math.abs(eb.getY()-iron.getY())<=iron.getWidth()/2){//子弹击中铁块
				...
				for(int k=0;k<myTank.getBullets().size();k++){
					...
					for(int t=0;t<enemyTank.getBullets().size();t++){
						//判断是否击中敌人坦克的子弹
						if(Math.abs(mb.getX()-enemyTank.getBullets().get(t).getX())<=6&&Math.abs(mb.getY()-enemyTank.getBullets().get(t).getY())<=6){
							//子弹死亡、坦克也死亡
							mb.setLive(false);
					...
					for(int l=0;l<map.getBricks().size();l++){				//取出每个砖块对象与子弹比较
						Brick brick=map.getBricks().get(l);
						if(Math.abs(mb.getX()-brick.getX())<=brick.getWidth()/2&&Math.abs(mb.getY()-brick.getY())<=brick.getWidth()/2){//子弹击中砖块
							this.afterShotStuff(mb, brick, bombs,myTank);
						}
					}	
					for(int t=0;t<map.getIrons().size();t++){
						Iron iron=map.getIrons().get(t);
						if(Math.abs(mb.getX()-iron.getX())<=iron.getWidth()/2&&Math.abs(mb.getY()-iron.getY())<=iron.getWidth()/2){//子弹击中砖块
		...
		for(int i=0;i<enemys.size();i++){
			...
			for(int j=0;j<bricks.size();j++){
				if(enemyTank.Overlap(bricks.get(j), 20+10)==true){
					...
					if((Math.abs(bricks.get(j).getX()-enemyTank.getX())<=10&&(enemyTank.getDirect()==EnemyTank.SOUTH||enemyTank.getDirect()==EnemyTank.NORTH))||(Math.abs(bricks.get(j).getY()-enemyTank.getY())<=10&&(enemyTank.getDirect()==EnemyTank.EAST||enemyTank.getDirect()==EnemyTank.WEST))){
	...
	public void cleanAndCreat(Vector<MyTank> myTanks,Vector<EnemyTank> enemys,Map map,Vector<Bomb> bombs){
		for(int i=0;i<myTanks.size();i++){
			...
			if(myTank.isLive()==false){
				...
				this.beKilled++;
				if(this.myTankNum>=1){				//如果还有我的坦克就创建一个，刚开始面板上就创建了一个我的坦克，所以大于等于 1
